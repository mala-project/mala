...
! Modifications by Normand Modine.
!
!--------------------------------------------------------------------
SUBROUTINE local_dos (iflag, lsign, kpoint, kband, spin_component, &
  ...
  !
  USE io_global,            ONLY : stdout, ionode  !  Normand Modine Hack
  USE constants,            ONLY : rytoev          !  Normand Modine Hack
  USE kinds,                ONLY : DP
  ...
  IF ( ionode ) THEN
     WRITE( stdout,'(/5x,"Gaussian broadening: ngauss,degauss=",i4,f12.6/)') &
         ngauss, emax * rytoev
  ENDIF
  ngauss = 0
  IF ( ionode ) THEN
     WRITE( stdout,'(/5x, "Code hacked by Normand Modine to force ngauss =", i4/)') ngauss
  ENDIF
