...
def qpa_labels_raw(
    rank, nmax, lmax, mumax, k1max, k2max, k1min, k2min, lmin=1, L_R=0, M_R=0
):
    if rank >= 4:
        all_max_l = 12
        all_max_n = 12
        ...
        kvecs = [
            i
            for i in itertools.combinations_with_replacement(range(kmax), rank)
        ]
        # reduced_nvecs=get_mapped_subset(nvecs)
        all_lammps_labs = []
        all_not_compat = []
        ...
        # no reduction required for rank <= 3
        # use typical lexicographical ordering for such cases
        labels = generate_nl(
            rank,
            nmax,
